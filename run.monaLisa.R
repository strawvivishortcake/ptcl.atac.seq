...
for (file_name in matrix_files) {
  ...
  # Save the full (unfiltered) per-comparison result so downstream scripts can
  # build a combined multi-comparison heatmap without re-running monaLisa.
  saveRDS(
    list(
      comparison   = comparison,
      motif_name   = rowData(se)$motif.name,
      log2enr      = assays(se)$log2enr,
      bin_nochange = colData(se)$bin.nochange,
      disease_samples = grp1_cols,
      normal_samples  = grp2_cols
    ),
    file.path(fig_dir, paste0(comparison, "_monaLisa_result.rds"))
  )
}
